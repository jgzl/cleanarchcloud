...
@EnableConfigurationProperties(ExtendXssProperties.class)
@ConditionalOnProperty(prefix = ExtendXssProperties.PREFIX, name = "enabled", havingValue = "true", matchIfMissing = true)
public class ExtendXssAutoConfiguration implements WebMvcConfigurer {

	private final ExtendXssProperties xssProperties;

	@Bean
	@ConditionalOnMissingBean
	public XssCleaner xssCleaner(ExtendXssProperties properties) {
		return new DefaultXssCleaner(properties);
	}

	@Bean
	public FormXssClean formXssClean(ExtendXssProperties properties, XssCleaner xssCleaner) {
		return new FormXssClean(properties, xssCleaner);
	}

	@Bean
	public Jackson2ObjectMapperBuilderCustomizer xssJacksonCustomizer(ExtendXssProperties properties,
