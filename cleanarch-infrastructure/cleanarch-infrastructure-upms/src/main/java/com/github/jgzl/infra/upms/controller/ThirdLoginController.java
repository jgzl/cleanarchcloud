...
public class ThirdLoginController {
	...
	public String login(@PathVariable String type, AuthCallback callback, HttpServletRequest request, HttpServletResponse response) throws IOException {
		AuthRequest authRequest = factory.get(type);
			...
			if (userVo!=null) {
				if (socialUser==null) {
					...
					if (CollUtil.isEmpty(socialUsers)) {
						...
						return "redirect:"+String.format("/oauth/rebind?user_id=%s&third_login_id=%s&redirect_url=%s",userVo.getUserId(),uuid,redirectUrl);
					}else {
				...
				if (CollUtil.isEmpty(sysUsers)) {
					HashOperations<String, Object, Object> hashOperations = redisTemplate.opsForHash();
					hashOperations.put(CacheConstants.PREFIX+"thirdLogin"+CacheConstants.SPLIT+authUser.getSource(),uuid,JSONUtil.toJsonStr(socialUser));
					return "redirect:"+String.format("/oauth/bind?third_login_id=%s&redirect_url=%s",uuid,redirectUrl);
				}else {
					SysUser user = sysUsers.get(0);
					//跳到首页
					request.setAttribute("username",user.getUsername());
					request.setAttribute("redirectUrl",redirectUrl);
					return "forward:"+ PathConstants.LOGIN_MODULE_THIRD_PATH;
				}
			}
		}
		return "error";
