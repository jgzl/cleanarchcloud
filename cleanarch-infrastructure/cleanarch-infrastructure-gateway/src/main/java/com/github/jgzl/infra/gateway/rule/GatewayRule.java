...
import com.github.jgzl.infra.gateway.handler.domain.CommonRule;
...
public interface GatewayRule<T> {
    ...
    default T getByPath(StringRedisTemplate stringRedisTemplate, ServerHttpRequest request, GatewayRuleEnum gatewayRule) {
        ...
        for (Object object : objects) {
            ...
            if (ObjectUtil.isAllNotEmpty(rule.getStartTime(), rule.getEndTime())) {
            ...
            final boolean methodFilter = StrUtil.equals(rule.getMethod(), "ALL") || StrUtil.equalsIgnoreCase(rule.getMethod(), Objects.requireNonNull(httpMethod).name());
