...
/**
 * Created by lihaifeng on 2019/5/23.
 * by lihaifeng
 */
...
//    DnsResolver dnsResolver = SystemDefaultDnsResolver.INSTANCE;
    ...
    httpClientBuilder = httpClientBuilder
        .evictIdleConnections(initParams.getParams("EvictIdleConnectionsTime", int.class), TimeUnit.SECONDS);
    ...
    if (initParams.getParams("ConnectionTimeToLive", int.class) > 0) {
      ...
      httpClientBuilder = httpClientBuilder
          .setConnectionTimeToLive(initParams.getParams("ConnectionTimeToLive", int.class), TimeUnit.SECONDS);
    }
    if (initParams.getParams("RetryCount", int.class) > 0) {
      //设置重试次数，默认是3次，当前是禁用掉（根据需要开启） 0
      httpClientBuilder = httpClientBuilder
          .setRetryHandler(new DefaultHttpRequestRetryHandler(initParams.getParams("RetryCount", int.class), false));
    }
