...
public interface HttpClient extends Closeable {
   ...
   *   初始化参数
   */
  class InitMap extends HashMap<EnumHttpConnectParam, Object> {
    @Override
  ...
  class Params {
    ...
    public HttpEntity toEntity() {
      if (!this.contentType.equals(ContentType.MULTIPART_FORM_DATA)) {
        return EntityBuilder.create().setContentType(this.contentType).setContentEncoding("utf-8")
            .setText(this.toString()).build();
      } else {
