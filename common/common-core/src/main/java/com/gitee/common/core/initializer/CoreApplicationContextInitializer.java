...
import com.gitee.common.core.base.BsfEnvironmentEnum;
...
import com.gitee.common.core.util.ContextUtils;
...
public class CoreApplicationContextInitializer implements
    ...
    public void initialize(ConfigurableApplicationContext context) {
        ...
        if (!Strings.isEmpty(propertyValue) && !Strings.isEmpty(propertyValue2)) {
            ...
            LogUtils.info(CoreApplicationContextInitializer.class, CoreProperties.Project,
    ...
    private void optimize(ConfigurableEnvironment environment) {
        ...
        if (ContextUtils.isWeb()) {
            ...
            setDefaultProperty(CoreProperties.ServerTomcatMinSpaceThreads, ((int) (20 * (cpuCount / 2))) + "",
                "[自动化调优]");
            //tomcat 传输优化
            ...
            if (root != null) {
                ...
                if (file != null && file instanceof RollingFileAppender) {
                    ...
                    if (rollingPolicy != null && rollingPolicy instanceof SizeAndTimeBasedRollingPolicy) {
                        ...
                        ((SizeAndTimeBasedRollingPolicy) rollingPolicy).setTotalSizeCap(
                            FileSize.valueOf(environment.getProperty(CoreProperties.BsfLoggingFileTotalSize, "1GB")));
                    }
                }
            }
        }
    ...
    void setDefaultProperty(String key, String defaultPropertyValue, String message) {
        PropertyUtils.setDefaultInitProperty(CoreApplicationContextInitializer.class, CoreProperties.Project, key,
    ...
    void refreshConetext() {
        if (ContextUtils.getApplicationContext() != null) {
            if (ContextUtils.MainClass == null) {
                LogUtils.error(CoreApplicationContextInitializer.class, CoreProperties.Project, "重启失败",
                    new BsfException("检测到重启上下文事件,因无法找到启动类，重启失败!!!"));
                return;
            }
            val context = ContextUtils.getApplicationContext();
            ApplicationArguments args = context.getBean(ApplicationArguments.class);
            val waitTime = new Random(UUID.randomUUID().getMostSignificantBits())
                .nextInt(PropertyUtils.getPropertyCache(CoreProperties.BsfContextRestartTimeSpan, 10));
            Thread thread = new Thread(() -> {
                try {
                    ...
                    LogUtils.error(CoreApplicationContextInitializer.class, CoreProperties.Project, "重启失败",
                        new BsfException("根据启动类" + ContextUtils.MainClass.getName() + "动态启动main失败"));
                }
            });
