...
app.controller('SentinelClusterSingleController',
    ['$scope', '$stateParams', 'ngDialog',
      ...
      function ($scope, $stateParams, ngDialog, MachineService,
        ...
        function fetchMachineClusterState() {
          ...
          ClusterStateService.fetchClusterUniversalStateSingle($scope.app,
              mac[0], mac[1]).success(function (data) {
            if (data.code == 0 && data.data) {
              ...
              if ($scope.stateVO.server && $scope.stateVO.server.namespaceSet) {
                $scope.stateVO.server.namespaceSetStr = convertSetToString(
                    $scope.stateVO.server.namespaceSet);
              }
            } else {
              $scope.stateVO = {};
              if (data.code === UNSUPPORTED_CODE) {
                $scope.loadError = {
                  message: '机器 ' + mac[0] + ':' + mac[1]
                      + ' 的 Sentinel 客户端版本不支持集群限流，请升级至 1.4.0 以上版本并引入相关依赖。'
                }
              } else {
        ...
        function checkValidClientConfig(stateVO) {
          ...
          if (config.serverPort === undefined || config.serverPort <= 0
              || config.serverPort > 65535) {
            alert('请输入有效的 Token Server 端口');
            return false;
          }
          if (config.requestTimeout === undefined || config.requestTimeout
              <= 0) {
            alert('请输入有效的请求超时时长');
            return false;
          }
        ...
        function sendClusterClientRequest(stateVO) {
          ...
          ClusterStateService.modifyClusterConfig(request).success(
              function (data) {
                if (data.code == 0 && data.data) {
                  alert('修改集群限流客户端配置成功');
                  ...
                  if (data.code === UNSUPPORTED_CODE) {
                    alert('机器 ' + mac[0] + ':' + mac[1]
                        + ' 的 Sentinel 客户端版本不支持集群限流客户端，请升级至 1.4.0 以上版本并引入相关依赖。');
                  } else {
                    alert('修改失败：' + data.msg);
                  }
                }
        ...
        function checkValidServerConfig(stateVO) {
          ...
          if (stateVO.server.namespaceSetStr === undefined
              || stateVO.server.namespaceSetStr == '') {
            alert('请输入有效的命名空间集合（多个 namespace 以 , 分隔）');
            return false;
          }
          let transportConfig = stateVO.server.transport;
          if (transportConfig.port === undefined || transportConfig.port <= 0
              || transportConfig.port > 65535) {
            alert('请输入有效的 Token Server 端口');
            return false;
          }
          let flowConfig = stateVO.server.flow;
          if (flowConfig.maxAllowedQps === undefined || flowConfig.maxAllowedQps
              < 0) {
        ...
        function sendClusterServerRequest(stateVO) {
          ...
          request.namespaceSet = convertStrToNamespaceSet(
              stateVO.server.namespaceSetStr);
          ClusterStateService.modifyClusterConfig(request).success(
              function (data) {
                if (data.code == 0 && data.data) {
                  ...
                  if (data.code === UNSUPPORTED_CODE) {
                    alert('机器 ' + mac[0] + ':' + mac[1]
                        + ' 的 Sentinel 客户端版本不支持集群限流服务端，请升级至 1.4.0 以上版本并引入相关依赖。');
                  } else {
                    alert('修改失败：' + data.msg);
                  }
                }
