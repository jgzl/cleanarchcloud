...
app.controller('GatewayIdentityCtl',
    ['$scope', '$stateParams', 'IdentityService',
      'ngDialog', 'GatewayFlowService', 'GatewayApiService', 'DegradeService',
      'MachineService',
      '$interval', '$location', '$timeout',
      function ($scope, $stateParams, IdentityService, ngDialog,
          GatewayFlowService, GatewayApiService, DegradeService, MachineService,
          $interval, $location, $timeout) {
        ...
        $scope.addNewGatewayFlowRule = function (resource) {
          ...
          gatewayFlowRuleDialogScope.intervalUnits = [{val: 0, desc: '秒'},
            {val: 1, desc: '分'}, {val: 2, desc: '时'}, {val: 3, desc: '天'}];
          ...
          gatewayFlowRuleDialogScope.currentRule = {
            ...
            ip: mac[0],
            port: mac[1],
            resourceMode: gatewayFlowRuleDialogScope.apiNames.indexOf(resource)
            == -1 ? 0 : 1,
            resource: resource,
            interval: 1,
            intervalUnit: 0,
        ...
        function saveGatewayFlowRule() {
          if (!GatewayFlowService.checkRuleValid(
              gatewayFlowRuleDialogScope.currentRule)) {
            return;
          }
          GatewayFlowService.newRule(
              gatewayFlowRuleDialogScope.currentRule).success(function (data) {
            if (data.code === 0) {
        ...
        function saveDegradeRule() {
          if (!DegradeService.checkRuleValid(
              degradeRuleDialogScope.currentRule)) {
            return;
          }
          DegradeService.newRule(degradeRuleDialogScope.currentRule).success(
              function (data) {
                if (data.code == 0) {
        ...
        function queryIdentities() {
          ...
          IdentityService.fetchClusterNodeOfMachine(mac[0], mac[1],
              $scope.searchKey).success(
              function (data) {
                if (data.code == 0 && data.data) {
