var com_github_culmat_jsTreeTable = (function () {
  ...
  function renderTree(tree, childrenAttr, idAttr, attrs, renderer,
      tableAttributes) {
    childrenAttr = childrenAttr || 'children'
    idAttr = idAttr || 'id'
  ...
  function treeTable(table) {
    ...
    function getLevel(node) {
      ...
      if (parentID == undefined) {
        ...
        return getLevel($('tr[data-tt-id="' + parentID + '"]', table).first())
            + 1
      }
    }
    ...
    $("tr[data-tt-level]", table).each(function (index, tr) {
      ...
      if (tr.trChildren.length > 0) {
        td.prepend(
            $('<img id="state" style="cursor:pointer" src="' + imgCollapse
                + '"/>'))
      } else {
        td.prepend($('<span style="padding-left:16px;" /></span>'))
      }
      td.prepend($('<span style="padding-left:' + (15 * parseInt(level - 1))
          + 'px;" /></span>'))
  ...
  function appendTreetable(tree, options) {
    ...
    var initialExpandLevel = options.initialExpandLevel ? parseInt(
        options.initialExpandLevel) : -1
