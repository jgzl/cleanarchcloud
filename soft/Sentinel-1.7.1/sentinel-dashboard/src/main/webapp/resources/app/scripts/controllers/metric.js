...
app.controller('MetricCtl',
    ['$scope', '$stateParams', 'MetricService', '$interval', '$timeout',
      function ($scope, $stateParams, MetricService, $interval, $timeout) {
        ...
        $scope.initAllChart = function () {
          $.each($scope.metrics, function (idx, metric) {
            ...
            chart.line().position('timestamp*passQps').size(1).color(
                'green').shape('smooth');
            chart.line().position('timestamp*blockQps').size(1).color(
                'blue').shape('smooth');
        ...
        function queryIdentityDatas() {
          ...
          MetricService.queryAppSortedIdentities(params).success(
              function (data) {
                $scope.metrics = [];
                $scope.emptyObjs = [];
