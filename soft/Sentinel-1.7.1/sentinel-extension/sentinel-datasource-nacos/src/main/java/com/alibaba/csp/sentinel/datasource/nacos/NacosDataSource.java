...
public class NacosDataSource<T> extends AbstractDataSource<String, T> {
		...
		this.configListener = new Listener() {
			...
			public void receiveConfigInfo(final String configInfo) {
				RecordLog.info(String
						.format("[NacosDataSource] New property value received for (properties: %s) (dataId: %s, groupId: %s): %s",
	...
	private void loadInitialConfig() {
		try {
			...
			if (newValue == null) {
				RecordLog
						.warn("[NacosDataSource] WARN: initial config is null, you may have to check your data source");
			}
			getProperty().updateValue(newValue);
		} catch (Exception ex) {
