...
public class ZookeeperDataSourceTest {
	...
	public void testZooKeeperDataSource() throws Exception {
		...
		ReadableDataSource<String, List<FlowRule>> flowRuleDataSource = new ZookeeperDataSource<List<FlowRule>>(
				remoteAddress, path,
	...
	public void testZooKeeperDataSourceAuthorization() throws Exception {
		...
		if (stat == null) {
			...
			zkClient.create().creatingParentContainersIfNeeded().withACL(Collections.singletonList(acl))
					.forPath(path, null);
		}
	...
	private void publishThenTestFor(CuratorFramework zkClient, String path, String resourceName, long count)
			throws Exception {
	...
	public void testZooKeeperDataSourceSameZkClient() throws Exception {
		...
		ZookeeperDataSource<List<DegradeRule>> degradeRuleZkDataSource = new ZookeeperDataSource<>(remoteAddress,
				degradePath,
		...
		ZookeeperDataSource<List<FlowRule>> flowRuleZkAutoDataSource = new ZookeeperDataSource<List<FlowRule>>(
				remoteAddress,
		...
		ZookeeperDataSource<List<DegradeRule>> degradeRuleZkAutoDataSource = new ZookeeperDataSource<List<DegradeRule>>(
				remoteAddress,
